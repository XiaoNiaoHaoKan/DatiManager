...
async function listAccounts(req, res) {
  const accounts = await MarketplaceUser.find().select('-password').sort({ username: 1 });
  res.json(accounts);
}
...
async function createAccount(req, res) {
  ...
  const existing = await MarketplaceUser.findOne({ username });
  if (existing) {
...
async function deleteAccount(req, res) {
  const deleted = await MarketplaceUser.findOneAndDelete({ username: req.params.username });
  if (!deleted) {
...
async function adjustCredit(req, res) {
  ...
  const account = await MarketplaceUser.findOne({ username: req.params.username });
  if (!account) {
    return res.status(404).json({ error: 'Account non trovato.' });
  }
